...
test("successfully submitting the contact form", async ({ page }) => {
  ...
  await expect(
    page.getByText("Thanks! We've received your message.")
  ).toBeVisible();
});

// Next.js doesn't give us a good way to build this functionality
test.skip("filling out fields, then leaving and pressing back will keep the fields populated", async ({
  page,
}) => {
  await page.goto("/contact");
  ...
  await expect(
    page.getByRole("heading", { name: "About Pokeland" })
  ).toBeVisible();
  // return
  await page.goBack();
...
test("forgetting to select a favorite starter pokemon", async ({ page }) => {
  ...
  await expect(page.getByLabel("Charmander")).toHaveAttribute(
    "aria-invalid",
    "true"
  );
  await expect(page.getByLabel("Squirtle")).toHaveAttribute(
    "aria-invalid",
    "true"
  );
  await expect(page.getByLabel("Bulbasaur")).toHaveAttribute(
    "aria-invalid",
    "true"
  );
  await expect(page.getByLabel("Pikachu")).toHaveAttribute(
    "aria-invalid",
    "true"
  );

  await expect(
    page.getByText("Oops, we couldn't save the form.")
  ).toBeVisible();
  await expect(
    page.getByText('"Starter Pokemon" must contain at least 1 items')
  ).toBeVisible();
});
...
test("selecting only pikachu", async ({ page }) => {
  ...
  await expect(
    page.getByText("If you select Pikachu you must pick another one too")
  ).toBeVisible();
});
